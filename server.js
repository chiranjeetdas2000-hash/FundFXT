/* ============================================================
   FUNDFXT — RENDER COMPATIBILITY ENTRYPOINT
   File: render-entrypoint.js
   Purpose: Patches backend/server.js before startup
            - Database-first account rules
            - Persistent account favorites
            - Affiliate commission flow
            - Grouped USD P&L calculation
            - Partial close + TP/SL repairs
   ============================================================ */
...
// ---------------------------------------------------------------------------
// 1. LOCATE & READ BACKEND SERVER
// ---------------------------------------------------------------------------
const target = path.join(__dirname, 'backend', 'server.js');
let source = fs.readFileSync(target, 'utf8');

// ---------------------------------------------------------------------------
// 2. PATCH: Fix trade query to require OPEN status
// ---------------------------------------------------------------------------
const fixedTradeQuery =
...
source = source
  .split('\n')
  .map((line) =>
    line.includes('req.params.tradeId') && line.includes('const [trades]')
      ? fixedTradeQuery
      : line
  )
  .join('\n');
...
// ---------------------------------------------------------------------------
// 3. PATCH: On trade close, update both balance_cents AND equity_cents
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 4. PATCH: Expand payment status check to include account-created stages
// ---------------------------------------------------------------------------
source = source.replace(
  "status === 'PAYMENT_DONE' || status === 'PAYMENT_APPROVED'",
  "status === 'PAYMENT_DONE' || status === 'PAYMENT_APPROVED' || status === 'ACCOUNT_CREATED' || status === 'ACCOUNT_PROVIDED'"
);

// ---------------------------------------------------------------------------
// 5. PATCH: Replace P&L calculation with grouped USD-aware version
// ---------------------------------------------------------------------------
const groupedPL = `
...
source = source.replace(
  /function calculatePL\(symbol, side, entry, current, volume\) \{[\s\S]*?\n\}/,
  groupedPL
);

// ---------------------------------------------------------------------------
// 6. PATCH: Replace /api/accounts route with normalized account rules
// ---------------------------------------------------------------------------
...
try{
  ...
  for(const account of accounts){
    ...
    let config={};
    try{
      const[cfg]=await db.execute('SELECT * FROM challenge_configs WHERE model_key = ? LIMIT 1',[model]);
      config=cfg[0]||{};
    }catch(_){}

    const num=v=>v===null||v===undefined||v===''?null:Number(v);
    const pick=(...keys)=>{
      for(const k of keys)if(config[k]!==null&&config[k]!==undefined&&config[k]!=='')return config[k];
      return null;
    };
    const accountPick=(...keys)=>{
      for(const k of keys)if(account[k]!==null&&account[k]!==undefined&&account[k]!=='')return account[k];
      return null;
    };
    ...
    const phaseRaw=account.phase||account.account_phase||account.challenge_phase||(warrior?'Phase 1':direct?'Direct Funded':'—');
    ...
    const targetPercent=targetBps==null?null:targetBps/100;
    ...
    const consistencyLimit=num(accountPick('consistency_limit_percent'))??(consistencyBps==null?(warrior?30:null):consistencyBps/100);
    ...
    const minLot=num(accountPick('min_lot','min_lot_size'))??num(pick('min_lot','min_lot_size'))??0.01;
    const maxLot=num(accountPick('max_lot','max_lot_size'))??num(pick('max_lot','max_lot_size'))??2;
    const payoutDays=num(accountPick('payout_period_days','payout_days'))??num(pick('payout_period_days','payout_days'));
    ...
    const maxLimitCents=maxDDbps==null?null:Math.round((direct?hwm:initial)*maxDDbps/10000);
    const maxUsedCents=maxDDbps==null?0:Math.max(0,(direct?hwm:initial)-(direct?equity:balance));
    ...
    const targetProgress=resolvedTargetCents&&resolvedTargetCents>0
      ?Math.max(0,Math.min(100,profitAchievedCents/resolvedTargetCents*100))
      :null;
    ...
    normalized.push({
      ...
      account_profile:{
        accountType,
        fundingModel:direct?'Direct Funded':warrior?'Warrior':'Challenge',
        phase,
        ...
        rules:{
          ...
          tradesToday:Number(todayRows[0].count||0),
          openPositions:Number(openRows[0].count||0),
          maxOpenPositions:first(num(config.max_open_positions),num(account.max_open_positions)),
          minLot,maxLot,payoutPeriodDays:payoutDays,
          ...
          dailyDrawdownCents:dailyLimitCents,
          dailyDrawdownUsedCents:dailyUsedCents,
          dailyDrawdownRemainingCents:dailyLimitCents==null?null:Math.max(0,dailyLimitCents-dailyUsedCents),
          maximumDrawdownPercent:maxDDbps==null?null:maxDDbps/100,
          maxDrawdownPercent:maxDDbps==null?null:maxDDbps/100,
          maxDrawdownCents:maxLimitCents,
          maxDrawdownUsedCents:maxUsedCents,
          maxDrawdownRemainingCents:maxLimitCents==null?null:Math.max(0,maxLimitCents-maxUsedCents),
          profitTargetPercent:targetPercent,
          profitTargetCents:resolvedTargetCents,
          profitAchievedCents,
          targetProgressPercent:targetProgress
        }
      }
    });
  }
...
// ---------------------------------------------------------------------------
// 7. PATCH: Favorite pairs (per-account watchlist)
// ---------------------------------------------------------------------------
const favoriteFlow = `
(async()=>{
  try{
    await db.execute("CREATE TABLE IF NOT EXISTS account_favorite_pairs (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, account_id BIGINT NOT NULL, symbol VARCHAR(20) NOT NULL, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE KEY uq_account_symbol (account_id,symbol), KEY idx_account_favorites (account_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  }catch(e){
    console.error('Favorite-pair table init:',e.message);
  }
})();

app.get('/api/favorite-pairs',authenticateToken,async(req,res)=>{
  try{
    const[a]=await db.execute('SELECT id,account_code FROM accounts WHERE user_id=?',[req.userId]);
    const account=a.find(x=>String(x.account_code)===String(req.query.account_code))||a[0];
    if(!account)return res.status(404).json({error:'No trading account available'});
    const[rows]=await db.execute('SELECT symbol FROM account_favorite_pairs WHERE account_id=? ORDER BY id ASC',[account.id]);
    res.json({success:true,account_code:account.account_code,favorites:rows.map(r=>r.symbol)});
  }catch(e){
    res.status(500).json({error:e.message});
  }
});

app.put('/api/favorite-pairs',authenticateToken,async(req,res)=>{
  try{
    const[a]=await db.execute('SELECT id,account_code FROM accounts WHERE user_id=?',[req.userId]);
    const account=a.find(x=>String(x.account_code)===String(req.body.account_code))||a[0];
    if(!account)return res.status(404).json({error:'No trading account available'});
    const favorites=Array.isArray(req.body.favorites)
      ?[...new Set(req.body.favorites.map(x=>String(x).toUpperCase()).filter(Boolean))].slice(0,50)
      :[];
    await db.execute('DELETE FROM account_favorite_pairs WHERE account_id=?',[account.id]);
    for(const symbol of favorites){
      await db.execute('INSERT IGNORE INTO account_favorite_pairs(account_id,symbol) VALUES(?,?)',[account.id,symbol]);
    }
    res.json({success:true,account_code:account.account_code,favorites});
  }catch(e){
    res.status(500).json({error:e.message});
  }
});
...
// ---------------------------------------------------------------------------
// 8. PATCH: Affiliate stats + dashboard flow
// ---------------------------------------------------------------------------
const affiliateFlow = `
app.get('/api/affiliate/stats',authenticateToken,async(req,res)=>{
  try{
    const[users]=await db.execute('SELECT affiliate_code FROM users WHERE id=? LIMIT 1',[req.userId]);
    if(!users.length)return res.status(404).json({error:'User not found'});
    const code=users[0].affiliate_code;
    const[refRows]=await db.execute('SELECT COUNT(*) AS n FROM users WHERE referred_by_code=?',[code]);
    const[salesRows]=await db.execute("SELECT COUNT(*) AS n FROM affiliate_commissions WHERE affiliate_id=(SELECT id FROM affiliates WHERE user_id=? LIMIT 1)",[req.userId]);
    const[earnedRows]=await db.execute("SELECT COALESCE(SUM(commission_amount_cents),0) AS total FROM affiliate_commissions WHERE affiliate_id=(SELECT id FROM affiliates WHERE user_id=? LIMIT 1) AND status IN ('PENDING','APPROVED','PAID')",[req.userId]);
    const[pendingRows]=await db.execute("SELECT COALESCE(SUM(commission_amount_cents),0) AS total FROM affiliate_commissions WHERE affiliate_id=(SELECT id FROM affiliates WHERE user_id=? LIMIT 1) AND status='PENDING'",[req.userId]);
    res.json({
      success:true,
      total_referrals:Number(refRows[0]?.n||0),
      total_sales:Number(salesRows[0]?.n||0),
      total_earnings_cents:Number(earnedRows[0]?.total||0),
      pending_earnings_cents:Number(pendingRows[0]?.total||0)
    });
  }catch(e){
    res.status(500).json({error:e.message});
  }
...
app.get('/api/affiliate/dashboard',authenticateToken,async(req,res)=>{
  try{
    const[rows]=await db.execute('SELECT id,affiliate_code FROM users WHERE id=? LIMIT 1',[req.userId]);
    if(!rows.length)return res.status(404).json({error:'User not found'});
    const[commissions]=await db.execute('SELECT c.id,c.order_ref,c.model,c.paid_amount_cents,c.commission_amount_cents AS commission_cents,c.status,c.created_at,u.legal_name AS customer_name FROM affiliate_commissions c LEFT JOIN users u ON u.id=c.referred_user_id WHERE c.affiliate_id=(SELECT id FROM affiliates WHERE user_id=? LIMIT 1) ORDER BY c.created_at DESC',[req.userId]);
    const totals=commissions.reduce((a,c)=>{
      const v=Number(c.commission_cents||0);
      a.total+=v;
      if(String(c.status).toUpperCase()==='PENDING')a.pending+=v;
      return a;
    },{total:0,pending:0});
    res.json({
      success:true,
      affiliate_code:rows[0].affiliate_code||null,
      total_earnings_cents:totals.total,
      available_earnings_cents:totals.pending,
...
// ---------------------------------------------------------------------------
// 9. INJECT: Affiliate flow + favorite flow before WebSocket server
// ---------------------------------------------------------------------------
const marker = '// ========== WEBSOCKET SERVER ==========';
source = source.replace(marker, affiliateFlow + favoriteFlow + marker);

// ---------------------------------------------------------------------------
// 10. PATCH: Partial close route (if not already present)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 11. PATCH: TP/SL modify route (if not already present)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 12. WRITE PATCHED SOURCE BACK TO server.js
// ---------------------------------------------------------------------------
fs.writeFileSync(target, source, 'utf8');
console.log(
  'FundFXT: database-first account rules + persistent account favorites + affiliate commission flow + grouped USD P&L + partial close + TP/SL repairs applied before startup.'
);

// ---------------------------------------------------------------------------
// 13. START THE PATCHED BACKEND
// ---------------------------------------------------------------------------
require('./backend/server.js');

// ---------------------------------------------------------------------------
// 14. RUN AFFILIATE RECONCILIATION AFTER STARTUP (5 second delay)
// ---------------------------------------------------------------------------
setTimeout(async () => {
  try {
    const { reconcileAffiliateCommissions } = require('./backend/affiliate-reconciliation');
    const result = await reconcileAffiliateCommissions();
    console.log('FundFXT affiliate reconciliation:', JSON.stringify(result));
  } catch (e) {
    console.error('FundFXT affiliate reconciliation startup error:', e.message);
  }
}, 5000);
