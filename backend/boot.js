...
if(!src.includes('FUNDFXT_BOOT_V6')){
  ...
  const commissionParts=[needle,"    if(order.affiliate_code){","      const [a]=await connection.execute('SELECT af.id AS affiliate_id FROM affiliates af JOIN users u ON u.id=af.user_id WHERE u.affiliate_code=? LIMIT 1',[order.affiliate_code]);","      if(a.length){","        const commissionCents=Math.floor((Number(order.final_amount_cents||0)*0.20)+100);","        const [existing]=await connection.execute('SELECT id FROM affiliate_commissions WHERE order_id=? LIMIT 1',[id]);","        if(!existing.length){","          await connection.execute(\"INSERT INTO affiliate_commissions (affiliate_id,order_id,referred_user_id,model,commission_amount_cents,status) VALUES (?,?,?,?,?,'PENDING')\",[a[0].affiliate_id,id,order.user_id,order.model,commissionCents]);","          await connection.execute('UPDATE affiliates SET total_sales=total_sales+1,total_earnings_cents=total_earnings_cents+?,pending_earnings_cents=pending_earnings_cents+? WHERE id=?',[commissionCents,commissionCents,a[0].affiliate_id]);","        }","      }","    }"].join('\n');
  src=src.replace(needle,commissionParts);

  // Intercept the payment-link endpoint before the legacy handler. This guarantees the
  // old status=link_sent SQL can never execute and LINK_SENT is written safely.
  const intercept=`
const FUNDFXT_V6_PAYMENT_LINK_INTERCEPT = async (req,res,next) => {
  ...
  try{
    ...
    const o=rows[0],adminEmail='support.fundfxt@gmail.com';
    ...
    const html=\`<div style="font-family:Arial,sans-serif;background:#f5f7f9;padding:28px;color:#17202a"><div style="max-width:680px;margin:auto;background:#fff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden"><div style="background:#0f0f0f;padding:22px 26px;color:#fff;font-size:24px;font-weight:800">Fund<span style="color:#00b56a">FXT</span></div><div style="padding:26px"><h2>Payment Link Request</h2><p>A customer has requested a FundFXT challenge payment link. Please review the details and forward the link to the customer.</p><table style="width:100%;border-collapse:collapse"><tr><td>Request ID</td><td>\${esc(o.order_ref)}</td></tr><tr><td>Customer Name</td><td>\${esc(o.legal_name)}</td></tr><tr><td>Customer Email</td><td>\${esc(o.user_email)}</td></tr><tr><td>Customer Phone</td><td>\${esc(o.user_phone)}</td></tr><tr><td>Customer Address</td><td>\${esc(o.user_address)}</td></tr><tr><td>Challenge</td><td>\${esc(o.model)}</td></tr><tr><td>Original Amount</td><td>\${money(o.original_amount_cents,o.currency)}</td></tr><tr><td>Discount</td><td>\${money(o.discount_amount_cents,o.currency)}</td></tr><tr><td>Final Payable</td><td><b>\${money(o.final_amount_cents,o.currency)}</b></td></tr><tr><td>Affiliate Code</td><td>\${esc(o.affiliate_code||'None')}</td></tr><tr><td>Affiliate Name</td><td>\${esc(o.affiliate_name||'—')}</td></tr></table><p><b>Razorpay Payment Link:</b><br><a href="\${esc(paymentLink)}">\${esc(paymentLink)}</a></p><p style="font-size:12px;color:#667085">Security notice: Trust payment-related emails only from <b>support.fundfxt@gmail.com</b>.</p></div></div></div>\`;
...
`;
  src=src.replace("app.use(express.json());","app.use(express.json());\n"+intercept+"app.use(FUNDFXT_V6_PAYMENT_LINK_INTERCEPT);");
  const tmp=path.join(os.tmpdir(),'fundfxt-start-v6.js');fs.writeFileSync(tmp,src);require(tmp);
