...
function installDatabaseControl(app) {
  ...
  app.post("/api/admin/database/payment-requests/:id/link",authenticateDatabaseAdmin,async(req,res)=>{
    ...
    try{
      ...
      let emailSent=false;
      try{emailSent=await sendSupportEmail("support.fundfxt@gmail.com",`FundFXT Payment Link — ${request.request_id} — Customer: ${request.legal_name}`,`<h2>FundFXT Payment Link</h2><p><b>Request ID:</b> ${request.request_id}</p><p><b>Customer Name:</b> ${request.legal_name||"—"}</p><p><b>Customer Email:</b> ${request.user_email||"—"}</p><p><b>Challenge:</b> ${request.model}</p><p><b>Payable:</b> ${(Number(request.final_amount_cents||0)/100).toFixed(2)} ${request.currency}</p><hr><p><b>Razorpay Link to forward to customer:</b></p><p><a href="${link}">${link}</a></p><hr><p>Please forward this link to the customer email: ${request.user_email}</p>`));}catch(error){console.error("Payment link email failed:",error.message);}
