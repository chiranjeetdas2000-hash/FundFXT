...
// Loaded with NODE_OPTIONS on Render so /api/accounts always exposes authoritative
// account metadata, live balances and rule progress without fabricated targets.
...
// Final source-layer repairs. The Render service starts through the compatibility
// entrypoint, so this runs again when backend/server.js is actually required.
fs.readFileSync = function patchedReadFileSync(file, encoding, ...rest) {
  ...
  const canonicalRoute = `app.get('/api/accounts',authenticateToken,async(req,res)=>{try{const[accounts]=await db.execute('SELECT * FROM accounts WHERE user_id = ?',[req.userId]);res.json({accounts})}catch(error){res.status(500).json({error:error.message})}});`;
  source = source.replace(/app\.get\('\/api\/accounts',authenticateToken,async\(req,res\)=>\{[\s\S]*?\}\);/, canonicalRoute);
  return source;
};
...
mysql.createPool = function patchedCreatePool(...args) {
  ...
  pool.execute = async function patchedExecute(sql, params) {
    ...
    for (const account of accounts) {
      ...
      let cfg = {};
      try { const [cfgRows] = await rawExecute('SELECT * FROM challenge_configs WHERE model_key=? LIMIT 1', [model]); cfg = cfgRows[0] || {}; } catch (_) {}
      const pick = (...keys) => firstValue(...keys.map(k => cfg[k] ?? account[k]));
      const warrior = model === 'warrior_5k';
      const direct = model === 'prototype_5k';
      const phase = String(firstValue(account.phase, account.account_phase, account.challenge_phase, warrior ? 'Phase 1' : direct ? 'Direct Funded' : '—'));
      const phase2 = /(?:^|[^0-9])2(?:$|[^0-9])|phase\s*2/i.test(phase);

      // Never invent a profit target. Show null / Not configured until the database supplies one.
      ...
      const targetBpsRaw = pick(phase2 ? 'phase2_profit_target_bps' : 'phase1_profit_target_bps', phase2 ? 'profit_target_phase2_bps' : 'profit_target_phase1_bps', 'profit_target_bps', 'target_bps', 'profit_target_percent_bps');
      ...
      const targetPercent = targetCents !== null ? (initial > 0 ? targetCents / initial * 100 : null) : targetBps !== null ? targetBps / 100 : targetPercentField;
      ...
      const consistencyLimit = consistencyBps !== null ? consistencyBps / 100 : consistencyPctField !== null ? consistencyPctField : (warrior ? 30 : null);
      ...
      const [todayRows] = await rawExecute('SELECT COUNT(*) AS count FROM trades WHERE account_id=? AND trading_day=CURDATE()', [id]);
      const [openRows] = await rawExecute("SELECT COUNT(*) AS count FROM trades WHERE account_id=? AND status='OPEN'", [id]);
      const profitAchievedCents = Math.max(0, balance - initial);
      const targetProgressPercent = resolvedTarget !== null && resolvedTarget > 0 ? Math.max(0, Math.min(100, profitAchievedCents / resolvedTarget * 100)) : null;
