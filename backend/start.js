...
if(!src.includes('FUNDFXT_RUNTIME_PATCH_V1')){
  ...
  src=src.replace("const [rows] = await db.query('SELECT * FROM settings');\n        const settings = {};\n        rows.forEach(row => {\n            settings[row.setting_key] = JSON.parse(row.setting_value);\n        });","const [rows] = await db.query('SELECT * FROM settings');\n        const settings = {};\n        rows.forEach(row => { const raw=row.setting_value ?? row.value ?? row.config ?? row.data; let parsed=raw; if(typeof raw==='string'){try{parsed=JSON.parse(raw)}catch(_){}} settings[row.setting_key]=parsed; });");
