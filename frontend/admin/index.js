...
async function loadAccountPayouts(status="PENDING"){currentAccountPayoutStatus=status;const body=$("accountPayoutsBody"),loading=$("accountPayoutsLoading"),empty=$("accountPayoutsEmpty"),errorEl=$("accountPayoutsError");loading.style.display="block";empty.style.display="none";errorEl.innerHTML="";body.innerHTML="";try{const d=await get("/api/admin/account-payouts?status="+encodeURIComponent(status)),rows=d.payouts||[];loading.style.display="none";if(!rows.length){empty.style.display="block";return;}body.innerHTML=rows.map(p=>{const st=String(p.status||"").toUpperCase(),badge=st==="APPROVED"?"green":st==="REJECTED"?"red":"yellow",action=st==="PENDING"?'<button class="btn" onclick="approveAccountPayout('+p.id+')">Approve</button> <button class="btn red" onclick="rejectAccountPayout('+p.id+')">Reject</button>':'<span class="badge '+badge+'">'+esc(st)+'</span>';return '<tr><td>'+esc(p.request_ref||"—")+'</td><td><b>'+esc(p.legal_name||"—")+'</b><br><small>'+esc(p.email||"")+'</small></td><td>'+esc(p.account_code||"—")+'</td><td>'+money(p.amount_cents)+'</td><td>'+(p.created_at?new Date(p.created_at).toLocaleDateString():"—")+'</td><td><span class="badge '+badge+'">'+esc(st)+'</span></td><td>'+action+'</td></tr>';}).join("");}catch(e){loading.style.display="none";errorEl.innerHTML='<div class="error-box">'+esc(e.message)+'</div>';}}
async function approveAccountPayout(id){if(!confirm("Approve this account profit payout? The full profit will be credited to the user's FundFXT Wallet and the account will reset."))return;try{await post("/api/admin/account-payouts/"+id+"/approve",{});toast("Account payout approved");loadAccountPayouts(currentAccountPayoutStatus);}catch(e){toast(e.message,"err");}}
async function rejectAccountPayout(id){const reason=prompt("Enter rejection reason:");if(!reason)return;try{await post("/api/admin/account-payouts/"+id+"/reject",{rejection_reason:reason});toast("Account payout rejected");loadAccountPayouts(currentAccountPayoutStatus);}catch(e){toast(e.message,"err");}}
