...
function phaseSnapshot(row){return row?.target_snapshot&&typeof row.target_snapshot==="object"?row.target_snapshot:{};}
function phaseReviewDetailHtml(d){const s=d?.review?.target_snapshot||{},ts=Array.isArray(d?.trades)?d.trades:[];const trades=ts.length?ts.map(t=>"<tr><td>"+esc(t.trade_id||"—")+"</td><td>"+esc(t.symbol||"—")+"</td><td>"+esc(t.side||"—")+"</td><td>"+esc(t.order_type||"—")+"</td><td>"+esc(t.volume??"—")+"</td><td>"+esc(t.entry_price??"—")+"</td><td>"+esc(t.exit_price??"—")+"</td><td>"+money(t.realized_profit_cents??0)+"</td><td>"+esc(t.close_reason||"—")+"</td><td>"+esc(t.exit_time||"—")+"</td></tr>").join(""):"<tr><td colspan=\"10\">No eligible closed trades found at pass time.</td></tr>";return '<div class="panel" style="margin:8px 0 12px;padding:14px;"><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin-bottom:14px;"><div><small>Profit at Target</small><br><b>'+money(s.profit_cents)+'</b></div><div><small>Target Value</small><br><b>'+money(s.target_cents)+'</b></div><div><small>Balance</small><br><b>'+money(s.balance_cents)+'</b></div><div><small>Equity</small><br><b>'+money(s.equity_cents)+'</b></div><div><small>Closed at Pass</small><br><b>'+esc(s.open_trades_closed??0)+'</b></div><div><small>Pending Cancelled</small><br><b>'+esc(s.pending_orders_cancelled??0)+'</b></div></div><div style="overflow:auto;"><table class="table"><thead><tr><th>Trade</th><th>Symbol</th><th>Side</th><th>Type</th><th>Volume</th><th>Entry</th><th>Exit</th><th>P/L</th><th>Reason</th><th>Closed</th></tr></thead><tbody>'+trades+'</tbody></table></div></div>';}
async function togglePhaseReviewDetail(id){const el=$("prDetail"+id);if(!el)return;if(el.style.display!=="none"){el.style.display="none";return;}el.style.display="table-row";el.querySelector(".phase-detail-content").innerHTML='<div class="loading">Loading review details…</div>';try{const d=await get("/api/admin/phase-reviews/"+id);el.querySelector(".phase-detail-content").innerHTML=phaseReviewDetailHtml(d);}catch(e){el.querySelector(".phase-detail-content").innerHTML='<div class="error-box">'+esc(e.message)+'</div>';}}
function renderPhaseReviewRow(row){const st=String(row.status||"").toUpperCase(),snap=phaseSnapshot(row),badge=st==="APPROVED"?"green":st==="REJECTED"?"red":"yellow",name=row.user?.name||row.user_name||"—",email=row.user?.email||row.user_email||"",userId=row.user?.id??row.user_id??"—",code=row.account?.code||row.account_code||"—",model=row.account?.challenge_model||row.challenge_model||"—",size=row.account?.initial_balance_cents??row.initial_balance_cents??0,action=st==="PENDING"?'<button class="btn" onclick="event.stopPropagation();approvePhaseReview('+row.id+')">Approve</button> <button class="btn red" onclick="event.stopPropagation();rejectPhaseReview('+row.id+')">Reject</button>':'<span class="badge '+badge+'">'+esc(st)+'</span>';return '<tr onclick="togglePhaseReviewDetail('+row.id+')" style="cursor:pointer;"><td><b>'+esc(row.review_ref||"—")+'</b></td><td><b>'+esc(name)+'</b><br><small>'+esc(email)+'</small></td><td>'+esc(userId)+'</td><td>'+esc(code)+'</td><td>'+esc(model)+'</td><td>'+money(size)+'</td><td>'+esc((row.from_phase||"—")+" → "+(row.to_phase||"—"))+'</td><td>'+money(snap.profit_cents)+'</td><td>'+money(snap.target_cents)+'</td><td>'+esc(row.created_at?new Date(row.created_at).toLocaleString():"—")+'</td><td><span class="badge '+badge+'">'+esc(st||"—")+'</span></td><td>'+action+'</td></tr><tr id="prDetail'+row.id+'" style="display:none;"><td colspan="12"><div class="phase-detail-content"></div></td></tr>';}
async function loadPhaseReviews(status="PENDING"){currentPhaseReviewStatus=status;["Pending","Approved","Rejected"].forEach(x=>{const e=$("prTab"+x);if(e)e.className=x.toUpperCase()===status?"btn":"btn secondary";});const body=$("phaseReviewsBody"),loading=$("phaseReviewsLoading"),empty=$("phaseReviewsEmpty"),errorEl=$("phaseReviewsError");if(!body)return;loading.style.display="block";empty.style.display="none";errorEl.innerHTML="";body.innerHTML="";try{const d=await get("/api/admin/phase-reviews?status="+encodeURIComponent(status));const rows=Array.isArray(d.reviews)?d.reviews:[];window.__phaseReviewRows=rows;loading.style.display="none";if(!rows.length){empty.style.display="block";return;}body.innerHTML=rows.map(renderPhaseReviewRow).join("");}catch(e){loading.style.display="none";errorEl.innerHTML='<div class="error-box">'+esc(e.message)+'</div>';}}
async function approvePhaseReview(id){const row=(window.__phaseReviewRows||[]).find(x=>Number(x.id)===Number(id));const next=row?.to_phase||"the next phase";if(!confirm("Approve this phase review and create "+next+" account?"))return;try{const d=await post("/api/admin/phase-reviews/"+id+"/approve",{});toast("Phase review approved. New account: "+(d.new_account_code||d.new_account_id));await loadPhaseReviews(currentPhaseReviewStatus);}catch(e){toast(e.message,"err");}}
async function rejectPhaseReview(id){const reason=prompt("Enter rejection reason:");if(!reason||!reason.trim())return;try{await post("/api/admin/phase-reviews/"+id+"/reject",{admin_note:reason.trim()});toast("Phase review rejected.");await loadPhaseReviews(currentPhaseReviewStatus);}catch(e){toast(e.message,"err");}}
