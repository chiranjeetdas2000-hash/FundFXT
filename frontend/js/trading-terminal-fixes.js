...
function rules(a){
  ...
  const initial=n(a.initial_balance_cents),balance=n(a.balance_cents),equity=n(a.equity_cents),size=n(a.account_size_cents),phase=p.phase??a.phase??a.account_phase;
...
function install(){
  ...
  document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||b.disabled||b.matches('.right-tab,#accountMenuBtn,.account-popover-close,.account-dashboard-btn,.mobile-nav button'))return;b.classList.add('fx-busy');setTimeout(()=>b.classList.remove('fx-busy'),900)},true);
