/* ===== FUNDFXT TERMINAL FIXES — ACCOUNT RULES ===== */
(function(){
  'use strict';
  ...
  window.fetch=async function(input,init){
    ...
    try{
      ...
      const account=selectedAccount(list);if(!account)return response;
      const trades=await tradesFor(account),x=ledger(account,trades);
  ...
  function accountRules(a,trades){
    ...
    return `<div class="account-rules account-rules-detailed">
      ...
      <div class="rule-grid">
        ${rule('Account type',esc(type))}${rule('Funding model',esc(funding))}${rule('Phase / stage',esc(phase))}${rule('Challenge model',esc(model))}${rule('Direct funded',direct?'Yes':'No')}
        ...
        ${warrior?rule('Phase 1 profit target','8% · '+money(phase1Cents)):''}
        ...
        ${direct?rule('Max payout','100% · one-time'):''}
      </div>
      ${targetCents!=null?bar('Current phase profit target progress',targetProgress):''}
  ...
  async function getAccount(){
    const d=await rawJson('/api/accounts'),list=d.accounts||[],a=selectedAccount(list);if(!a)throw Error('No trading account available');
    const t=await tradesFor(a),x=ledger(a,t);return {...a,balance_cents:x.balance,equity_cents:x.equity,__trades:t};
  }
  async function renderAccountPopover(){
    document.querySelector('.account-popover')?.remove();
    const a=await getAccount(),trades=a.__trades||[];localStorage.setItem('fundfxt_selected_account',a.account_code);
    ...
    document.body.appendChild(el);el.querySelector('.account-popover-close').onclick=()=>el.remove();
  }
  ...
  function install(){
    ...
    document.querySelectorAll('.right-tab').forEach(btn=>btn.addEventListener('click',()=>setActiveTradeTab(btn.dataset.tab),true));setActiveTradeTab('OPEN');
    document.addEventListener('click',e=>{const p=document.querySelector('.account-popover');if(p&&!p.contains(e.target)&&e.target!==btn)p.remove()});setInterval(refreshAccountPopover,1500);
  }
