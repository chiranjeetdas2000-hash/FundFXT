...
function showDetails(id) {
    ...
    if(!t)return;
    const cents=Number(t.realized_profit_cents||0);
    const fields=[['Trade ID',t.trade_id],['Pair',t.symbol],['Direction',t.side],['Lot Size',Number(t.volume).toFixed(2)],['Open Time',t.entry_time||t.created_at||'—'],['Close Time',t.exit_time||'—'],['Entry Price',fmt(Number(t.entry_price),t.symbol)],['Exit Price',fmt(Number(t.exit_price),t.symbol)],['Take Profit',(Number(t.take_profit)>0)?fmt(Number(t.take_profit),t.symbol):'Not set'],['Stop Loss',(Number(t.stop_loss)>0)?fmt(Number(t.stop_loss),t.symbol):'Not set'],['Exit By',t.close_reason||'—'],['Calculated P/L',t.status==='CLOSED'?`${cents>=0?'+':''}${money(cents)}`:'Open / Floating']].map(([a,b])=>`<div class="detail"><span>${a}</span><b>${b??'—'}</b></div>`).join('');
