    ...
    if(url.startsWith(API+'/')){
      ...
      if(url===API+'/api/prices'){
        try{
          ...
          const backendPrices=payload?.prices&&typeof payload.prices==='object'?payload.prices:{};
          const backendSymbols=Array.isArray(payload?.symbols)?payload.symbols:SYMBOLS;
          const missing=backendSymbols.some(s=>!backendPrices[s] || !Number.isFinite(Number(backendPrices[s]?.bid)) || !Number.isFinite(Number(backendPrices[s]?.ask)));
          if(!response.ok || missing){
            const direct=await directPrices();
            const merged={...backendPrices};
            for(const symbol of Object.keys(direct)){
              const q=direct[symbol];
              if(q&&Number.isFinite(Number(q.bid))&&Number.isFinite(Number(q.ask)))merged[String(symbol).toUpperCase()]=q;
            }
            return new Response(JSON.stringify({success:true,source:response.ok?'BiQuote-merged':'BiQuote-direct',symbols:[...new Set([...backendSymbols,...Object.keys(merged)])],prices:merged}),{status:200,headers:{'Content-Type':'application/json'}});
          }
        }catch(e){
          try{
            const direct=await directPrices();
