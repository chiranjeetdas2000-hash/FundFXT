...
// ===================== WEBSOCKET (FCS API via Backend) =====================
function connectWebSocket() {
    const ws = new WebSocket("wss://fundfxt.onrender.com/ws");
    ...
    ws.onmessage = (event) => {
        ...
        // Update live prices in sidebar & topbar
    ...
    ALL_PAIRS.forEach(pair => {
        ...
        li.innerHTML = `
            <div class="pair-header">
                ...
                <span class="fav ${state.favorites.includes(pair) ? 'active' : ''}" 
                      onclick="toggleFavorite('${pair}', event)">★</span>
            </div>
            <div class="price-row">
                <div class="bid-box">
...
function updateSidebarPrices() {
    for (const pair in state.livePrices) {
        ...
        if (bidEl && askEl) {
            bidEl.textContent = tick.bid ? tick.bid.toFixed(5) : '--';
            askEl.textContent = tick.ask ? tick.ask.toFixed(5) : '--';
            
            if (spreadEl && tick.spread) {
                spreadEl.textContent = tick.spread;
            }
            
            if (changeEl && tick.change_pct) {
...
function updateCurrentPrice() {
    ...
    if (tick) {
        ...
        if (currentPriceEl && tick.bid) {
            currentPriceEl.textContent = tick.bid.toFixed(5);
        }
    }
}
...
function executeOrder(side) {
    ...
    alert(`${side.toUpperCase()} Order Placed: ${lots} lots at ${entryPrice}`);
}
...
function validateLotSize(input) {
    ...
    if (parseFloat(value) < 0.01) value = '0.01';
    if (parseFloat(value) > 2) value = '2.00';
    input.value = value;
}
